...
2. Authentication → Providers → Email : activer, **magic link** ; désactiver « confirm email » n'est pas nécessaire (invitation). Le gabarit *Magic Link* doit contenir `{{ .Token }}` : c'est lui qui affiche le code à six chiffres (D11). Authentication → Emails → SMTP : un vrai serveur d'envoi est indispensable, l'expéditeur intégré est plafonné à quelques mails par heure.
3. Authentication → Providers → Google : non activé, et le bouton a été retiré de l'écran de connexion (D11). À rouvrir le jour où un client OAuth Google Cloud existe.
...
**Sociétés** (`#societes`, entre Semaine et À qualifier — lot Sociétés, maquette `design/couverture.html`) : une ligne par couple *(société × prospecteur)*, lue dans `v_comptes`. Barre d'outils sur deux lignes — « Comptes de » (Toute l'équipe + un bouton par responsable réellement présent) et recherche société ou contact ; « Situation » (Toutes + une pastille par situation avec le nombre de **comptes**, grisée à zéro, + « Aucun échange »). Bandeau de cinq chiffres cliquables : comptes attribués (dédoublonnés sur `company_id`) · jamais contactés par leur prospecteur · sans appel depuis 14 j et plus · contacts jamais appelés · comptes avec un échange. Tableau : Compte (nom serif · secteur · phrase « où on en est » tronquée, infobulle complète) · Prospecteur (+ « aussi X » si le compte est partagé) · Contacts appelés (jauge, rouge pleine si zéro) · Dernier appel du prospecteur (« il y a N j » ; sinon **jamais** en rouge et « X y est allé le … ») · Situation la plus chaude. Tri : À faire en premier (défaut), Prospecteur, Dernier appel, Nom. Fiche à droite, **fermée par défaut** : nom, secteur, compte de X, nombre de contacts ; « Où on en est » signé de la routine ; trois chiffres ; les contacts filtrables en trois groupes (appelés par le propriétaire · par quelqu'un d'autre · jamais appelés) avec, sous chacun, le fil de ses appels (date · collaborateur · situation ou « Pas de décroché » · résumé · « À faire : » · Écouter · Fiche Jarvi). Depuis la fiche appel de la page Jour, le bouton **Voir le compte** ouvre cette fiche.
